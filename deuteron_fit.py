...
def _fit_func_complex(w, A, G, r, wQ, wL, eta, xi, phase):
    ...
    Both components are computed analytically from the Dulya integrals (_f for
    absorption, _f(..., disp=True) for its Kramers-Kronig partner), then combined as
    cos(phase)*absorption + sin(phase)*dispersion.
    """
    R = (w - wL) / (3 * wQ)
...
def _f(R, A, eps, eta, disp=False):
    ...
    For eta < 0.001 (axially symmetric), the phi average is trivial. Otherwise,
    integrates _integrals over phi in [0, pi] using a Romberg scheme: endpoints
    are accumulated first, then interior points are added at successive halvings
    from order 2 to 5 (4 to 32 new points). The sqrt(3)/Y weight is the powder
    density of states. eps=+1 selects the upper transition, eps=-1 the lower.
    disp=True computes the dispersion component instead of absorption.
    """
    if eta < 0.001:
...
def _integrals(R, A, eps, Y2, etac2p, disp=False):
    """Single-orientation Dulya integrand for absorption (disp=False) or dispersion (disp=True).

    Absorption evaluates A/(z²+A²); dispersion evaluates its Kramers-Kronig
    partner z/(z²+A²). Both use the same half-angle decomposition of the complex
    Lorentzian pole (cosa_2, sina_2). For dispersion, sina_2 and cosa_2 are
    swapped in T and L, and the normalization changes from 2*qq to pi*qq.
    """
    Y = np.sqrt(Y2)
    Yx2 = 2 * Y
    z2 = 1 - eps * R - etac2p
